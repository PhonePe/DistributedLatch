...
public class DistributedLatchFactoryTest {
    ...
    public void setUp() {
        ...
        countDownLatch = DistributedLatchFactory.createCountDownLatch(
    ...
    @Test
    public void testCreateCountDownLatch() {
        ...
        countDownLatch = DistributedLatchFactory.createCountDownLatch(
                CLIENT_ID, LatchLevel.DC, FARM_ID, LATCH_ID, storageContext, count);
        DistributedLatchFactory.createCountDownLatch(
                CLIENT_ID, LatchLevel.DC, FARM_ID, LATCH_ID, storageContext, count);
    ...
    @Test
    public void testCreateCountUpDownLatch() {
        Map<String, Object> values = new HashMap<>();
        values.put(AEROSPIKE_COUNT_BIN, count);
        Record mockedRecord = new Record(values, 0, 0);
        Mockito.when(storageContext.getAerospikeClient().get(Mockito.any(Policy.class), Mockito.any(Key.class)))
        ...
        countUpDownLatch = DistributedLatchFactory.createCountUpDownLatch(
                CLIENT_ID, LatchLevel.DC, FARM_ID, LATCH_ID, storageContext, count);
        Assert.assertNotNull(countUpDownLatch);
    }

    @Test
    public void testGetOrCreateCountUpDownLatch() {
        Map<String, Object> values = new HashMap<>();
        values.put(AEROSPIKE_COUNT_BIN, count);
        Record mockedRecord = new Record(values, 0, 0);
        Mockito.when(storageContext.getAerospikeClient().get(Mockito.any(Policy.class), Mockito.any(Key.class)))
    ...
    @Test
    public void testGetCountDownLatch() {
        Map<String, Object> values = new HashMap<>();
        values.put(AEROSPIKE_COUNT_BIN, count);
        Record mockedRecord = new Record(values, 0, 0);
        Mockito.when(storageContext.getAerospikeClient().get(Mockito.any(Policy.class), Mockito.any(Key.class)))
                .thenReturn(mockedRecord);
        countDownLatch = DistributedLatchFactory.getCountDownLatch(
                CLIENT_ID, LatchLevel.DC, FARM_ID, LATCH_ID, storageContext);
        Assert.assertNotNull(countDownLatch);
    }

    @Test
    public void testGetCountUpDownLatch() {
        Map<String, Object> values = new HashMap<>();
        values.put(AEROSPIKE_COUNT_BIN, count);
        Record mockedRecord = new Record(values, 0, 0);
        Mockito.when(storageContext.getAerospikeClient().get(Mockito.any(Policy.class), Mockito.any(Key.class)))
                .thenReturn(mockedRecord);
        countUpDownLatch = DistributedLatchFactory.getCountUpDownLatch(
                CLIENT_ID, LatchLevel.DC, FARM_ID, LATCH_ID, storageContext);
        Assert.assertNotNull(countUpDownLatch);
    }
}
